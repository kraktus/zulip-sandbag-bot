...
pub fn repo_dir() -> PathBuf {
    ...
    pub async fn req(client: &Client, builder: RequestBuilder, auth: Option<String>) -> Response {
        ...
        loop {
            match req_inner(client, builder.try_clone().expect("No streaming body"), &auth)
                .await
            {
                Ok(resp) => return resp,
                Err(err) => error!(
    ...
    async fn req_inner(client: &Client, mut builder: RequestBuilder, auth: &Option<String>) -> Result<Response, Error> {
